...
tc_content = u"회원가입 시 이메일이 공백일 경우 '이메일을 입력해주세요.'라는 문구가 출력되어야 한다."
...
class UntitledTestCase(unittest.TestCase):
    ...
    def test_untitled_test_case(self):
        ...
        driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div/div[3]/div[2]/input").send_keys("qw")
        time.sleep(2)
        driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div/div[3]/div[2]/input").send_keys(Keys.CONTROL + "a")
        driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div/div[3]/div[2]/input").send_keys(Keys.DELETE)
        time.sleep(2)
        
        element = driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div[1]/div[3]/div[2]/div").text
        if(element!=u"이메일을 입력해주세요."):
            test_details = u"이메일 공백 메세지가 뜨지 않음"
            assert False
        else:
            print("SUCCESS")
