...
tc_content = u"회원가입 시 아이디가 공백일 경우 '아이디에 공백을 제거 또는 아이디를 입력해주세요.'라는 문구가 출력되어야 한다."
...
class UntitledTestCase(unittest.TestCase):
    ...
    def test_untitled_test_case(self):
        ...
        print("STEP 2 -- 아이디 공백 검사")
        driver.find_element_by_xpath("//input[@type='text']").click()
        driver.find_element_by_xpath("//input[@type='text']").send_keys("te")
        time.sleep(3)   
        driver.find_element_by_xpath("//input[@type='text']").send_keys(Keys.CONTROL + "a")
        driver.find_element_by_xpath("//input[@type='text']").send_keys(Keys.DELETE)
        time.sleep(3)
        
        element = driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div/div/div[2]/div").text
        if(element != u"아이디에 공백을 제거 또는 아이디를 입력해주세요."):
            test_details = u"아이디 공백 메세지가 뜨지 않음"
            assert False
        else:
            print("SUCCESS")
