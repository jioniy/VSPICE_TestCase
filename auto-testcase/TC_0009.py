...
tc_content = u"회원가입 시 비밀번호와 비밀번호 확인 란의 입력이 일치하지 않는 경우 '비밀번호가 동일하지 않습니다.'라는 문구가 출력되어야 한다. "
...
class UntitledTestCase(unittest.TestCase):
    ...
    def test_untitled_test_case(self):
        ...
        driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div[2]/div[2]/div[2]/input").send_keys("1234")
        time.sleep(1)
        
        element = driver.find_element_by_xpath("//div[@id='userInfoVue']/div/div[3]/div[2]/div[2]/div[2]/div").text
        if(element!=u"비밀번호가 동일하지 않습니다."):
            test_details += u"비밀번호와 비밀번호 확인 입력이 일치하지 않을 때 메세지가 뜨지 않음\n"
        else:
            print("SUCCESS")
        time.sleep(2)
